...
This directory contains the Hugo configuration for the go-yaml documentation site.
...
├── go-yaml.yaml.template       # Dynamic config template (version info)
├── gendoc.go                    # Local development server (go run gendoc.go)
├── themes/
│   ├── hugo-relearn/          # Relearn theme (extracted from zip)
│   ├── go-yaml-assets/        # Custom SCSS and assets
│   │   └── _custom.scss       # Theme customization
│   └── go-yaml-static/        # Static files (logo, favicon)
...
2. Generates `go-yaml.yaml` from template
...
- Custom SCSS in `themes/go-yaml-assets/_custom.scss`
- Static branding assets in `themes/go-yaml-static/`
...
2. **go-yaml.yaml** - Dynamic configuration (version, build time)

The dynamic config is generated at build time from `go-yaml.yaml.template`.
...
- Publish to: https://go-openapi.github.io/go-yaml/
